...
def plot_model_r2(model, data, lims=None, size=1, device="cpu"):
    pred = model(data, device=device).cpu().detach().numpy()
...
def eval_model(model, data, device="cpu"):
    ''' 
...
def eval_model_all(model, dataset, device="cpu"):
    vals = []
    for data in dataset:
        vals.append(eval_model(model, data, device=device))
    vals = np.array(vals)
    return vals

def eval_model_multiple(model, datasets, device="cpu"):
    vals = dict()
    for key in datasets:
        vals[key] = eval_model_all(model, datasets[key], device=device)
    return vals
