...
def plot_data(data, colors="black", color_bounds=None, label = None, size=30, width=1, colorbar=True, cmap="jet"):
    xy = data.x[:,:2].cpu().detach().numpy()
    node_color = colors.cpu().detach().numpy() if type(colors) == torch.Tensor else colors
...
def plot_comparison(model, shapes, filename=None, dpi=300, size=15, device="cpu"):
    ...
    for i, data in enumerate(shapes):
        gt = data.y
        pred = model(data, device=device).cpu()
        s = size / (1 + 7*(3000<data.x.shape[0]))
        maxval = max(torch.max(gt).cpu().detach().numpy(), torch.max(pred).cpu().detach().numpy())
        maxerr = np.max(torch.abs(gt.cpu()-pred.cpu()).detach().numpy())

        plt.subplot(N,4,1+i*4)
        plot_data(data, data.y, size=s, label="Ground Truth", color_bounds=[0,maxval])

        plt.subplot(N,4,2+i*4)
        plot_data(data, pred, size=s, label="Prediction", color_bounds=[0,maxval])
